...
export default function FromEmailList() {
    ...
    const [filteredEmails, setFilteredEmails] = useState<FromEmail[]>([]);
    ...
    const saveEmails = (updatedEmails: FromEmail[]) => {
    ...
    const EmailRow = ({ item, index }: { item: FromEmail; index: number }) => (
    ...
    const MobileCard = ({ item, index }: { item: FromEmail; index: number }) => (
        ...
        <div className="bg-[#e5e7eb] p-0 sm:p-5 min-h-screen flex justify-center items-start font-['Segoe_UI',Tahoma,Geneva,Verdana,sans-serif]">
            <div className="bg-white w-full max-w-[1400px]">
                ...
                <div className="w-full px-4 sm:px-6 py-6 pb-8">
                            ...
                            <thead>
                                <tr className="bg-[#dee2e6]">
                                    ...
                                    {['SR. NO.', 'EMAIL', 'DELETE'].map((h, i) => (
                                        <th 
                                            key={h} 
                                            className={`py-3 px-4 text-${i === 2 ? 'right' : 'left'} text-xs font-semibold text-[#6c757d] uppercase tracking-wider ${i < 2 ? 'border-r' : ''} border-b border-gray-300 ${i === 0 ? 'w-24' : ''}`}
                                        >
                                            {h}
                                        </th>
                                    ))}
                            ...
                            <tbody className="bg-white">
                                {filteredEmails.length === 0 ? (
                                    ...
                                    filteredEmails.map((item, index) => (
                                        <EmailRow key={item.id} item={item} index={index} />
                                    ))
                                )}
                            </tbody>
                        </table>
                    ...
                    <div className="mt-6">
                        <button 
                            className="bg-red-500 text-white px-6 py-2 rounded text-sm hover:bg-red-600 w-full sm:w-auto transition-colors"
