...
const ProductsTable: React.FC = () => {
  ...
  const fetchProducts = async (): Promise<void> => {
    ...
    try {
      ...
      const result: ApiResponse<Product[]> = await apiGet("/api/manage-items/products/get-products");
      ...
      alert((err as Error).message || "Failed to load products");
    } finally {
      setLoading(false);
    }
  };
  ...
  // ✅ CRITICAL: Update with tenant validation
  ...
  const handleSaveProduct = async (): Promise<void> => {
    ...
    try {
      ...
      const result: ApiResponse = await apiPost("/api/manage-items/products/create-product", {
        name: newProduct,
      });
      ...
      alert((err as Error).message || "Failed to add product");
    }
  };
