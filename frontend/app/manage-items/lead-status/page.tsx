...
export default function LeadStatusPage() {
  ...
  const fetchStatuses = async (): Promise<void> => {
    ...
    try {
      ...
      const result: ApiResponse<LeadStatus[]> = await apiGet("/api/manage-items/lead-status/get-lead-status");
  ...
  const handleAddLeadStatus = async (): Promise<void> => {
    ...
    try {
      ...
      const result: ApiResponse<LeadStatus> = await apiPost("/api/manage-items/lead-status/create-lead-status", {
  ...
  const filteredLeads: LeadStatus[] = leadStatuses.filter((l) =>
