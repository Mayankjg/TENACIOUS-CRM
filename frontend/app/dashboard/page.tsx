  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const leadStats: LeadStats = {
        ...
        closed: allLeads.filter((l: any) => l.leadStatus === "Closed").length,
        open: allLeads.filter((l: any) => l.leadStatus === "Open").length,
      ...
      if (user.role === "admin") {
        ...
        salesPersonsData = salespersons.map((sp: any) => {
          ...
          const todayLeads = spLeads.filter((l: any) => {
            const createdDate = new Date(l.createdAt);
            createdDate.setHours(0, 0, 0, 0);
            return createdDate.getTime() === today.getTime();
          });
          ...
          return {
            ...
            all: spLeads.length,
            missed: spLeads.filter((l: any) => l.leadStatus === "Miss").length,
            unscheduled: spLeads.filter(
              (l: any) =>
                l.leadStatus === "Unscheduled" || l.category === "Unscheduled"
            ).length,
            closed: spLeads.filter((l: any) => l.leadStatus === "Closed").length,
            void: spLeads.filter((l: any) => l.leadStatus === "Void").length,
          };
        });
      }
