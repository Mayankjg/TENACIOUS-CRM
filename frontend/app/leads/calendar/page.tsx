...
const LeadsCalendar = () => {
  ...
  const fetchLeads = async () => {
    try {
      setLoading(true);
      const res = await axios.get<Lead[] | { data?: Lead[]; leads?: Lead[] }>(
        `${API_BASE}/api/leads/get-leads`
      );
      const payload = res.data;
      const items: Lead[] = Array.isArray(payload)
        ? payload
                ...
                <div className="min-w-[400px]">
                  ...
                  <div className="grid grid-cols-8 bg-[#e5e7eb] border-b border-gray-300 bg-opacity-90">
                    ...
                    {weekDays.map((day, dayIndex) => {
                      ...
                      return (
                        <div
                          key={dayIndex}
                          className="text-center min-h-[50px] sm:min-h-[60px] p-2 border-r border-gray-300 bg-white/80 hover:bg-gray-50 transition-colors flex flex-col items-center justify-center"
                        >
                  ...
                  <div className="grid grid-cols-8 border-b border-gray-300">
                    ...
                    {weekDays.map((day, dayIndex) => (
                      <div
                        key={dayIndex}
                        className="min-h-[40px] sm:min-h-[50px] p-2 border-r border-gray-300 bg-white/80 hover:bg-gray-50 transition-colors"
                      ></div>
                    ))}
                  </div>
                  ...
                  <div className="overflow-y-auto max-h-[500px]">
                    {timeSlots.map((time, timeIndex) => (
                      ...
                      >
                        <div className="text-left py-4 px-2 text-xs sm:text-sm text-gray-600 border-r border-gray-300 bg-gray-50/80">
                          {time}
                        </div>
                        {weekDays.map((day, dayIndex) => {
                          ...
                          return (
                            <div
                              key={dayIndex}
                              className="min-h-[60px] p-2 border-r border-gray-300 bg-white/80 hover:bg-blue-50/50 transition-colors cursor-pointer relative"
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-semibold text-gray-800">
                  Leads for {selectedTimeSlot.day.toLocaleDateString()} at{" "}
                ...
                {selectedTimeSlot.leads.map((lead, idx) => (
                  ...
                  >
                    <div className="grid grid-cols-2 gap-2 text-sm text-black">
                      ...
                      <div>
                        <strong>Product:</strong> {lead.product || "N/A"}
                      </div>
                    </div>
                  </div>
                ))}
