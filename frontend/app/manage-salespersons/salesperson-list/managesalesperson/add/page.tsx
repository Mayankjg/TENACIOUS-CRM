...
//         // Optional: alert("Failed to load countries");
//         setLoadingCountries(false);
//       }
//     };
...
//     // SPECIFIC LOGIC FOR COUNTRY CHANGE
//     if (name === "country") {
...
//         ...prev,
//         country: value,
//         countryCode: newCode, // Auto-fill country code
//       }));
//     } else {
...
//   // ------------------------------------------------
//   // SAVE BUTTON - CORRECTED API ROUTE
//   // ------------------------------------------------
...
//     try {
//       const res = await fetch(`${API_BASE}/salespersons/create-salesperson`, {
//         method: "POST",
//         body: form,
//       });
...
//           <button
//             onClick={handleSave}
//             className="px-8 cursor-pointer py-2 text-white bg-cyan-500 rounded hover:bg-cyan-600"
//           >
//             Save
//           </button>
...
//             className="px-6 py-2 cursor-pointer text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50"
//           >
//             Cancel
//           </button>
//         </div>
...
export default function AddSalespersonForm() {
  ...
  // ------------------------------------------------
  // SAVE BUTTON - WITH IMAGE HANDLING
  // ------------------------------------------------
  const handleSave = async (): Promise<void> => {
    if (!validateForm()) {
      alert("Please fill in all required fields correctly.");
    ...
    if (formData.profileImage) {
      form.append("profileImage", formData.profileImage);
      console.log("Profile image added to form:", formData.profileImage.name);
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || "Failed to save salesperson");
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-sm text-black">
        ...
        <div className="border-t bg-[#e5e9ec] border-gray-200 px-6 py-4 flex justify-end gap-3">
          ...
          <button
            onClick={handleSave}
            className="px-8 cursor-pointer py-2 text-white bg-cyan-500 rounded hover:bg-cyan-600 transition-colors"
          >
            Save
          </button>
          ...
          <button
            onClick={handleCancel}
            className="px-6 py-2 cursor-pointer text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50 transition-colors"
          >
            Cancel
          </button>
        </div>
